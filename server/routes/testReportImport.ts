...
const { extractPdfText, parseTestReport, severityFor } = require('../lib/testReportParse');
...
// ── POST /preview ─────────────────────────────────────────────────────────────
router.post('/preview', requireManager, upload.single('file'), async (req: any, res: any) => {
  try {
    ...
    let assetMatch = null;
    if (parsed.meta.serialNumber) {
      const a = await prisma.asset.findFirst({
        where: { accountId: req.user.accountId, archivedAt: null,
                 serialNumber: { equals: parsed.meta.serialNumber, mode: 'insensitive' } },
    ...
    const summary = {
      total: m.length,
      red:    m.filter((x: any) => x.passFail === 'RED').length,
      yellow: m.filter((x: any) => x.passFail === 'YELLOW').length,
      green:  m.filter((x: any) => x.passFail === 'GREEN').length,
      deficienciesToCreate: m.filter((x: any) => severityFor(x.passFail, x.critical)).length,
    };
    return res.json({ success: true, data: { meta: parsed.meta, assetMatch, measurements: m, detectedLabels: parsed.detectedLabels, summary } });
  } catch (err) {
