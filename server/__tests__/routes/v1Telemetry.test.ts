...
describe('Phase 4 #8 telemetry ingestion', () => {
  ...
  test('CRIT reading auto-creates a QuoteRequest (triggerType=TELEMETRY_CRIT)', async () => {
    const quotes = await prisma.quoteRequest.findMany({
      where: { accountId: admin.accountId, assetId, triggerType: 'TELEMETRY_CRIT' },
    });
    expect(quotes.length).toBe(1);
    expect(quotes[0].driver).toBe('suspected_failing');
    expect(quotes[0].timeline).toBe('within_1_week');
    expect(quotes[0].status).toBe('requested');
    expect(quotes[0].requestedById).toBeTruthy();
    const snapshot = quotes[0].dossierSnapshot as any;
    expect(snapshot?.autoGenerated).toBe(true);
    expect(snapshot?.source).toBe('telemetry_crit');
  });
  ...
  test('manual acknowledge clears a CRIT notification and Condition 2', async () => {
    // Breach again (re-escalate) — auto-quote idempotency check must not create a duplicate.
    ...
    // Idempotency: still only 1 QuoteRequest even after re-escalation
    const quotes = await prisma.quoteRequest.findMany({
      where: { accountId: admin.accountId, assetId, triggerType: 'TELEMETRY_CRIT' },
    });
    expect(quotes.length).toBe(1);
