...
function drawFooter(doc: any, meta: any, pageNum: number) {
  ...
  // The footer must be cursor-neutral: it draws below the bottom margin, so if it
  // left doc.y down there the next content write would auto-break to a new page.
  ...
  try {
    ...
    // Right-align by computed x (NOT a width + align:'right'): giving the text a
    // width lets pdfkit flow it, and a flow below the bottom margin auto-adds a
    // blank page. A bare x with lineBreak:false can never paginate.
    doc.fillColor(COLORS.textSubtle).font(FONT_REG).fontSize(8);
  ...
  } finally { doc.x = sx; doc.y = sy; doc._renderingFooter = false; }
}
...
function renderCfoReportPdf(data: any, meta: any): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    // Footer on every page. pageAdded fires for pages 2+; page 1 is drawn
    // explicitly below while it is still the current page — drawing it at the end
    // would stamp a stale "Page 1" onto whatever page the cursor finished on.
    let pageNum = 1;
