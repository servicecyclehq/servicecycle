...
| CC6.4 | Manages changes to access | User activation/deactivation; instant token revocation on password reset; SCIM-brokered directory sync via SSO (Ory Polis implements SCIM against the IdP; ServiceCycle consumes provisioning/deprovisioning events over an inbound webhook, not a self-hosted SCIM v2 resource server) | `server/routes/auth.ts` `tokenEpoch`; `server/routes/ssoScim.ts` webhook consumer | — |
...
| CC6.8 | Manages encryption keys and access monitoring | AES-256-GCM for per-account secrets; `ENCRYPTED_KEYS` env var; backup crypto; every authenticated `/api/v1` call logged to the tamper-evident activity log (`action=api_v1_call`) with key name/ID, method, path, HTTP status, latency, and client IP; admin enabling/disabling field encryption writes `encryption_enabled`/`encryption_disabled` events (CEF sev 7) to the tamper-evident audit log; key rotation runbook in `docs/KEY_ROTATION.md` (zero-downtime JWT dual-verify, `MASTER_KEY` re-encrypt, `BACKUP_ENCRYPTION_KEY` rotation) | `server/lib/docCrypto.ts`; `server/lib/backupCrypto.ts`; `server/middleware/apiKeyAuth.ts`; `server/routes/settings.ts`; `docs/KEY_ROTATION.md` | — |
...
| CC7.3 | Evaluates security events to determine if they are security incidents | Per-email lockout (5 failures / 15 min → 15-min lock); per-IP rate limit; every failure writes `login_failed` (CEF severity 6) to activity chain; lockout trigger writes distinct `login_lockout_triggered` (CEF severity 7) event queryable by admin | `server/routes/auth.ts` `_recordLoginFail`, `EMAIL_LOCKOUT_*`; `login_failed` + `login_lockout_triggered` in activityLog | — |
...
| C1.2 | Disposes of confidential information | Account export + deletion path; GDPR deletion-on-request via `support@servicecycle.app`. Export includes all structured data: assets, work orders, deficiencies, quote requests, arc-flash studies + labels, LOTO procedures, parts catalog, spare inventory, and asset part requirements. **Automated retention enforcement** via nightly prune cascade (03:00–03:55 UTC) covering activity_logs (365d), notification_logs (180d), backup_logs (180d), refresh_tokens (30d), webhook_dlq (30d), telemetry_readings (365d), extraction_events (180d), render_errors (30d), ai_usage (90d), and demo account inactivity. Each prune job wraps in `runOnce()` and pings healthchecks.io on completion | `GET /api/export/account`; `docs/OFFBOARDING.md`; `server/index.ts` cron registrations; `docs/security/RETENTION_ENFORCEMENT_DESIGN.md` §Actual state; `server/lib/activityLogPrune.ts`; `server/lib/backupLogPrune.ts`; `server/lib/webhookDlqPrune.ts` | — |
...
4. ~~**Data retention enforcement**~~ — ✅ **CLOSED (2026-07-04 sweep discovery)**. `server/index.ts` already registers a full nightly cascade of prune crons (03:00–03:55 UTC): activity_logs 365d, notification_logs 180d, backup_logs 180d, refresh_tokens 30d, webhook_dlq 30d, telemetry_readings 365d, extraction_events 180d, render_errors 30d, ai_usage 90d, plus demo inactivity prune. Documented in `docs/security/RETENTION_ENFORCEMENT_DESIGN.md`. Closes C1.2 gap.
