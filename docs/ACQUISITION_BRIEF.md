...
**Full-fidelity native DWG + DGN rendering (ODA Drawings SDK license)** — ServiceCycle ships PDF-first with best-effort DWG conversion via open-source tools (LibreOffice + LibreDWG) for the electrical one-line EDMS module. This is architecturally correct for the pilot NETA-contractor ICP (who receives PDFs 95%+ of the time). Enterprise buyers with heavy CAD-native workflows require full-fidelity DWG + DGN rendering — the productized upgrade is a $7,500 year-1 / $4,500/yr recurring Open Design Alliance Drawings SDK Sustaining tier license, which covers DWG + DGN + IFC + STEP + 3D-PDF in one fee with no per-conversion cost and SaaS use explicitly permitted. The DrawingConverter adapter interface in the EDMS module is designed for this swap: LibreOffice today, ODA on a feature flag. **The deferral is deliberate:** we chose to defer the ODA licensing spend until a paying customer required native-DWG fidelity. Autodesk Platform Services was evaluated and explicitly excluded from the architecture (ToS §7.1 restricts "automated translation service" use, plus a ~3.3x price increase and mandatory hub migration in December 2025, plus multi-region data residency on Autodesk's cloud — all wrong shape for SC's SOC-2 posture). See `docs/scoping/EDMS_MODULE_SCOPE_2026-07-04.md` §17 for full analysis.
