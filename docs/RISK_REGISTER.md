...
| R-03 | VPS compromise / single point of failure | Infrastructure | 3 | 4 | 12 | DigitalOcean SOC 2 certified DC; daily in-app Postgres backup cron with 30-day retention; swap + memory limits via Docker Compose; `docs/DEPLOY_RUNBOOK.md`. **Backup destination is operator-configured: `BACKUP_DEST=local` by default (on-box only) and `s3` only when `BACKUP_S3_*` are set — offsite is NOT guaranteed unless `s3` is configured and verified on the box.** Verify via the boot log / `/api/admin/backup-config` before claiming offsite. | 6 | Engineering | Active — enforce `BACKUP_DEST=s3` in prod + migrate to managed DB (PaaS) as scale grows |
...
| R-06 | Service outage / data loss during deploy | Operational | 3 | 3 | 9 | Zero-downtime JWT rotation (dual-verify window); DB migrations run in a separate gated `server-migrate` container before the server restarts; pre-deploy `pg_dump`; `docs/DEPLOY_RUNBOOK.md` rollback steps. **Not zero-downtime: `docker compose up -d` has a brief restart window (see DEPLOY_RUNBOOK §11) — acceptable for the current single-box demo/early-prod posture; blue-green is deferred.** | 4 | Engineering | Active |
...
| 2026-06-26 | Engineering | Cross-check pass against code/runbook: R-03 corrected to state backup destination is operator-configured (`local` by default, `s3` only when set), not guaranteed-offsite; R-06 corrected to state deploys are NOT zero-downtime (brief restart window per DEPLOY_RUNBOOK §11). Residual scores unchanged. |
