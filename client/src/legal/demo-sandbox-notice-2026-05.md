...
# ServiceCycle — Demo Sandbox Notice
...
ServiceCycle's demo sandbox is a public, shared environment for evaluating the product. It runs the same code as the on-prem release, pre-populated with sample contracts, vendors, and renewal scenarios.

## What you're agreeing to

By creating a sandbox account, you confirm you understand:

1. **No real data.** Don't upload, paste, or type real contracts, customer data, financial credentials, personally identifiable information about third parties, or anything else that's confidential, regulated, or under NDA. The demo is for clicking around — not for actual contract management. If you want to use ServiceCycle on real data, install it on your own infrastructure (the auto-reply email after our signup form ships you a one-line installer). We may delete or suspend any sandbox we reasonably believe contains data prohibited above; we are not obligated to notify you before doing so.
...
4. **AI features run on the demo's shared AI infrastructure, with strict per-user caps.** All AI features are capped per user, per day, on the demo. As of v0.35.0 (2026-05-17):
   ...
   - **Ask ServiceCycle chat queries** capped at **10 per day per user**.
...
- The submission is emailed to the ServiceCycle team.
...
- Submissions are used to improve ServiceCycle. They are not shared with third
  parties beyond the email service operator (Brevo) handling delivery.

Self-hosted production deployments of ServiceCycle have this feature **off by
