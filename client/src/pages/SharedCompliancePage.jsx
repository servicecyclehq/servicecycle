...
const SEV_COLOR = { critical: 'var(--color-danger, #b91c1c)', high: 'var(--color-warning, #c2410c)', medium: 'var(--color-warning, #b45309)', low: 'var(--color-success, #3f6212)' };
...
export default function SharedCompliancePage() {
  ...
  const wrap = { maxWidth: 760, margin: '0 auto', padding: '32px 20px', fontFamily: 'system-ui, sans-serif', color: 'var(--color-text, #0a0d12)' };
  ...
  if (error) return (
    <div style={wrap}>
      ...
      <p style={{ color: 'var(--color-text-secondary, #5b6373)', fontSize: 14 }}>Ask the facility to send you a fresh link.</p>
    </div>
  );
  if (!data) return null;
  ...
  if (data.kind === 'underwriting') {
    ...
    const uwColor = rd.overallRate == null ? 'var(--color-text-secondary, #64748b)' : rd.overallRate >= 90 ? 'var(--color-success, #15803d)' : rd.overallRate >= 70 ? 'var(--color-warning-strong, #92400e)' : 'var(--color-danger, #b91c1c)';
    const cell = { padding: '12px 14px', background: 'var(--color-bg-secondary, #f8fafc)', border: '1px solid var(--color-border, #e2e8f0)', borderRadius: 8 };
    return (
      <div style={wrap} className="print-doc">
        ...
        <div className="no-print" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}>
          ...
          <span style={{ fontSize: 12, color: 'var(--color-ai-emphasis)', fontWeight: 700 }}>READ-ONLY</span>
        </div>
        <div style={{ fontSize: 13, color: 'var(--color-text-secondary, #5b6373)', marginTop: 4 }}>{data.watermark}</div>
        <div style={{ fontSize: 13, color: 'var(--color-text-secondary, #5b6373)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 16, flexWrap: 'wrap', marginTop: 12 }}>
          <div style={{ fontSize: 48, fontWeight: 700, color: uwColor }}>{rd.overallRate == null ? '—' : `${rd.overallRate}%`}</div>
          <div style={{ fontSize: 13, color: 'var(--color-text-secondary, #5b6373)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
          <div style={cell}>
            <div style={{ fontSize: 22, fontWeight: 700 }}>{rp.totalFindings ?? 0}</div>
            <div style={{ fontSize: 12, color: 'var(--color-text-secondary, #5b6373)' }}>likely audit findings ({rp.categories ?? 0} categories)</div>
          ...
          <div style={cell}>
            <div style={{ fontSize: 22, fontWeight: 700 }}>{rp.untrackedAssets ?? 0}</div>
            <div style={{ fontSize: 12, color: 'var(--color-text-secondary, #5b6373)' }}>assets on no maintenance program</div>
          </div>
          <div style={cell}>
            <div style={{ fontSize: 22, fontWeight: 700 }}>{rp.forgottenAssets ?? 0}</div>
            <div style={{ fontSize: 12, color: 'var(--color-text-secondary, #5b6373)' }}>not serviced in 3+ yrs{rp.neverServiced > 0 ? ` (${rp.neverServiced} never)` : ''}</div>
          </div>
        </div>
          ...
          <ul style={{ paddingLeft: 18, lineHeight: 1.7, marginTop: 12 }}>
            {rp.topFindings.map((f, i) => (
              <li key={i} style={{ fontSize: 14 }}>
                <span style={{ color: SEV_COLOR[f.severity] || 'var(--color-text-secondary, #5b6373)', fontWeight: 700 }}>{f.title}</span>
                <span style={{ color: 'var(--color-text-secondary, #5b6373)' }}> — {f.count}</span>
              </li>
            ))}
          </ul>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
          <div style={cell}><div style={{ fontSize: 12, color: 'var(--color-text-secondary, #5b6373)' }}>1-year</div><div style={{ fontSize: 16, fontWeight: 700 }}>{fmtRange(fin.plan?.year1)}</div></div>
          <div style={cell}><div style={{ fontSize: 12, color: 'var(--color-text-secondary, #5b6373)' }}>3-year (cumulative)</div><div style={{ fontSize: 16, fontWeight: 700 }}>{fmtRange(fin.plan?.year3)}</div></div>
          <div style={cell}><div style={{ fontSize: 12, color: 'var(--color-text-secondary, #5b6373)' }}>5-year (cumulative)</div><div style={{ fontSize: 16, fontWeight: 700 }}>{fmtRange(fin.plan?.year5)}</div></div>
        </div>
        <div style={{ fontSize: 12, color: 'var(--color-text-secondary, #5b6373)', marginTop: 8 }}>
        ...
        <div style={{ marginTop: 24, ...cell }}>
          ...
          {ev.latestSnapshot ? (
            <>
              ...
              <div style={{ color: 'var(--color-text-secondary, #5b6373)', wordBreak: 'break-all', marginTop: 4, fontFamily: 'monospace', fontSize: 11 }}>SHA-256 {ev.latestSnapshot.sha256}</div>
            </>
          ) : ' No snapshot generated yet.'}
        </div>
        </section>

        <div style={{ marginTop: 24, fontSize: 11, color: 'var(--color-text-muted, #94a3b8)' }}>{data.disclaimer}</div>
        <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--color-border, #e2e8f0)', fontSize: 11, color: 'var(--color-text-muted, #94a3b8)' }}>
  ...
  const rateColor = rate == null ? 'var(--color-text-secondary, #64748b)' : rate >= 90 ? 'var(--color-success, #15803d)' : rate >= 70 ? 'var(--color-warning-strong, #92400e)' : 'var(--color-danger, #b91c1c)';
  ...
  return (
    <div style={wrap} className="print-doc">
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 16, flexWrap: 'wrap', marginTop: 12 }}>
        <div style={{ fontSize: 48, fontWeight: 700, color: rateColor }}>{rate == null ? '—' : `${rate}%`}</div>
        <div style={{ fontSize: 13, color: 'var(--color-text-secondary, #5b6373)' }}>
      ...
      {data.latestSnapshot && (
        ...
        <div style={{ marginTop: 18, padding: '12px 14px', background: 'var(--color-bg-secondary, #f8fafc)', border: '1px solid var(--color-border, #e2e8f0)', borderRadius: 8, fontSize: 13 }}>
          ...
          <div style={{ color: 'var(--color-text-secondary, #5b6373)', wordBreak: 'break-all', marginTop: 4, fontFamily: 'monospace', fontSize: 11 }}>
      ...
      <div style={{ marginTop: 36, paddingTop: 12, borderTop: '1px solid var(--color-border, #e2e8f0)', fontSize: 11, color: 'var(--color-text-muted, #94a3b8)' }}>
