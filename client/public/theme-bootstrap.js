/* v0.92.3: Pre-React theme bootstrap (externalized from index.html for CSP).
   Reads servicecycle_theme from localStorage (set by the ThemeToggle in the sidebar
...
(function () {
  try {
    ...
    // Authenticated app routes still read and apply servicecycle_theme normally.
    var p = location.pathname;
    var pub = ['/', '/login', '/register', '/forgot-password', '/reset-password', '/accept-invite'];
    if (pub.indexOf(p) !== -1 || p.indexOf('/legal/') === 0) return;
    var t = localStorage.getItem('servicecycle_theme');
    if (t === 'dark' || t === 'light') {
