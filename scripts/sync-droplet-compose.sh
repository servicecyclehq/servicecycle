...
#
# Sync docker-compose.ghcr.yml + docker-compose.demo.yml from /root/lapseiq-src/
# (the checked-out git working tree) to /root/lapseiq/ (the deploy directory),
# then force-recreate the server container so it picks up any environment
# block changes.
#
# Why this is its own script:
#   The standard deploy-vX.Y.Z-do-console.sh scripts bump LAPSEIQ_VERSION in
#   /root/lapseiq/.env and roll containers, but they don't copy the compose
...
echo "▶ [1/3] Copying compose files from /root/lapseiq-src/ to /root/lapseiq/"
cp /root/lapseiq-src/docker-compose.ghcr.yml /root/lapseiq/docker-compose.ghcr.yml
cp /root/lapseiq-src/docker-compose.demo.yml /root/lapseiq/docker-compose.demo.yml
echo "  ✓ compose files synced"

echo ""
echo "▶ [2/3] Force-recreate server container (.env passthrough refresh)"
docker compose -f /root/lapseiq/docker-compose.ghcr.yml -f /root/lapseiq/docker-compose.demo.yml up -d --pull never --force-recreate server
...
docker compose -f /root/lapseiq/docker-compose.ghcr.yml -f /root/lapseiq/docker-compose.demo.yml exec server env | grep -E "^(AI_|CF_|HF_|GROQ|ANTHROPIC|AI_BUDGET)" | sort
echo ""
echo "  /api/health:"
curl -sS https://demo.lapseiq.com/api/health
echo ""
