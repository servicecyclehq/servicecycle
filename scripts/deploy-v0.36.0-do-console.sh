#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
# v0.36.0 manual deploy — help drawer + opt-in brief sections — paste-ready for the DigitalOcean web console
...
echo ""
echo "▶ [1/14] Fetching ${VERSION} tag from origin"
cd /root/lapseiq-src
...
echo ""
echo "▶ [2/14] Building lapseiq-server:${VERSION}"
docker build \
...
echo ""
echo "▶ [3/14] Building client SPA (npm ci + npm run build with VITE_API_URL='')"
...
echo ""
echo "▶ [4/14] Building lapseiq-client:${VERSION} image"
docker build \
...
echo ""
echo "▶ [5/14] Bumping compose .env (with rollback target)"
cd /root/lapseiq
...
echo ""
echo "▶ [6/14] Verifying guide-sections shipped inside the v0.35.x server image"
...
echo ""
echo "▶ [7/14] Verifying v0.35.x guide + v0.36.0 help artifacts shipped in the image"
docker run --rm --entrypoint sh \
  "ghcr.io/claudedussy/lapseiq-server:${VERSION}" \
  -c 'test -f /app/lib/guideRetrieval.js && test -f /app/lib/helpRegistry.js && test -f /app/lib/pdfHelpDoc.js && test -f /app/routes/help.js' \
  || { echo "❌ Help-drawer artifacts missing from image. Aborting."; exit 1; }
HELP_SECTION_COUNT=$(docker run --rm --entrypoint sh \
  "ghcr.io/claudedussy/lapseiq-server:${VERSION}" \
  -c 'ls /app/data/help/*.txt 2>/dev/null | wc -l')
if [ "${HELP_SECTION_COUNT}" -ne 9 ]; then
  echo "❌ Expected 9 help .txt files inside /app/data/help/, found ${HELP_SECTION_COUNT}. Did help:sync run? Aborting."
  exit 1
fi
echo "  ✓ guideRetrieval / helpRegistry / pdfHelpDoc / routes/help present"
echo "  ✓ ${HELP_SECTION_COUNT} help-doc .txt files baked into the image"
docker run --rm --entrypoint sh \
  "ghcr.io/claudedussy/lapseiq-server:${VERSION}" \
  -c 'test -f /app/lib/aiBrief/optInSections.js' \
  || { echo "❌ aiBrief/optInSections.js missing from image. Aborting."; exit 1; }
echo "  ✓ aiBrief/optInSections.js present"

echo ""
echo "▶ [8/14] Running Prisma migrate deploy (adds renewalBriefSectionsHash column)"
# One-shot container picks up the new image's prisma client + migrations.
...
docker compose -f docker-compose.ghcr.yml -f docker-compose.demo.yml run --rm \
  --no-deps --entrypoint sh server -c 'npx prisma migrate deploy' \
  || { echo "❌ prisma migrate deploy failed. Aborting before container recreate."; exit 1; }
echo "  ✓ migrations applied"
echo ""
echo "▶ [9/14] Cloudflare Workers AI env vars (verify operator state)"
...
echo ""
echo "▶ [10/14] Rolling containers (--pull never — use locally-built images)"
...
echo ""
echo "▶ [11/14] Wait 25s, then re-seed the demo account so the new tile-coverage rows land"
sleep 25
...
echo ""
echo "▶ [12/14] Reset .env workarounds (v0.35.x morning cleanup)"
...
echo ""
echo "▶ [13/14] Smoke test — wait 20s then probe"
sleep 20
echo "  • /api/health:"
...
echo ""
echo "▶ [14/14] Verifying renewalBriefSectionsHash column landed"
docker compose -f docker-compose.ghcr.yml -f docker-compose.demo.yml exec -T db \
  psql -U lapseiq -d lapseiq -c "SELECT column_name FROM information_schema.columns WHERE table_name='"'"'contracts'"'"' AND column_name='"'"'renewalBriefSectionsHash'"'"';" \
  | grep -q renewalBriefSectionsHash \
  && echo "  ✓ renewalBriefSectionsHash column exists" \
  || echo "  ⚠️  renewalBriefSectionsHash column NOT visible — check migrations status"
...
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "Next: Dustin to spot-check the v0.36.0 demo at https://demo.lapseiq.com
  Help drawer:
    - Click any "?" icon next to a sidebar entry (Dashboard, Contracts, Reports, Vendors, Alerts, Settings)
    - Drawer should slide in from the right with the module's content + TOC
    - Try the "PDF" button in the drawer header — should download LapseIQ-Help-<slug>.pdf
    - "Ask LapseIQ →" footer button should close drawer + open the chat panel
  Renewal Brief Sections:
    - Settings → AI tab → scroll to "Renewal Brief Sections"
    - 3 toggles ON by default (license_utilization_analysis, coterm_opportunities, quote_request_hygiene)
    - 2 toggles OFF by default (recommended_strategy, internal_stakeholder_map)
    - In DEMO_MODE the toggles should be locked / read-only — that is by design
  Old v0.35.x checks (regression):"
echo "  Ask LapseIQ:"
